...
import 'package:adminlikhwao/Model/AdminDisputeOrderDetailsModel.dart';
...
class AdminDisputeOrderDetailsScreen extends StatefulWidget {
  final int disputeId;

  const AdminDisputeOrderDetailsScreen({
    super.key,
    required this.disputeId,
  });

  @override
  State<AdminDisputeOrderDetailsScreen> createState() =>
      _AdminDisputeOrderDetailsScreenState();
}

class _AdminDisputeOrderDetailsScreenState
    extends State<AdminDisputeOrderDetailsScreen> {
  late Future<AdminDisputeOrderDetailsModel> detailsFuture;
  ...
  void loadDetails() {
    detailsFuture = fetchOrderDetails(disputeId: widget.disputeId);
  }
  ...
  Future<AdminDisputeOrderDetailsModel> fetchOrderDetails({
    ...
    final response = await http.get(
      Uri.parse(
        "${apiConfig.baseUrl}/api/admin/disputes/$disputeId/order-details",
      ),
      headers: {
    ...
    if (response.statusCode == 200 && body["status"] == "success") {
      return AdminDisputeOrderDetailsModel.fromJson(body);
    }
  ...
  Future<bool> reviseDisputeApi({
    required int disputeId,
    required String adminDecisionNote,
  }) async {
    try {
      final user = FirebaseAuth.instance.currentUser;

      if (user == null) {
        showSnack("Admin not logged in", red);
        return false;
      }
      ...
      if (token == null || token.isEmpty) {
        showSnack("Token not found", red);
        return false;
      }

      final url = "${apiConfig.baseUrl}/api/admin/disputes/$disputeId/revise";

      print("ADMIN REVISE API: $url");
      ...
      print("ADMIN REVISE STATUS: ${response.statusCode}");
      print("ADMIN REVISE BODY: ${response.body}");
      ...
      if (response.statusCode == 200 && body["status"] == "success") {
        showSnack(
          body["message"]?.toString() ??
              "Revision requested from writer successfully",
          green,
        );
        return true;
      }
      ...
      showSnack(
        body["message"]?.toString() ?? "Unable to request revision",
        red,
      );
      return false;
    } catch (e) {
      print("ADMIN REVISE ERROR: $e");
      showSnack("Unable to request revision: $e", red);
      return false;
    }
  }

  Future<void> showReviseDialog(AdminDisputeOrderDetailsModel data) async {
          ...
          builder: (context, setDialogState) {
            return AlertDialog(
              ...
              shape: RoundedRectangleBorder(
                ...
                side: BorderSide(color: orange.withOpacity(0.45)),
              ),
              titlePadding: const EdgeInsets.fromLTRB(18, 18, 18, 0),
              contentPadding: const EdgeInsets.fromLTRB(18, 14, 18, 0),
              actionsPadding: const EdgeInsets.fromLTRB(14, 12, 14, 14),
              title: Row(
                children: [
                  Container(
                    height: 38,
              ...
              content: Column(
                ...
                children: [
                  Text(
                    "Write a clear note for the writer. This note will be saved and writer will be asked to upload revised work.",
                    style: TextStyle(
                      color: subText,
                      fontSize: 12.5,
                      height: 1.35,
                  ...
                  TextField(
                    ...
                    decoration: InputDecoration(
                      hintText:
                      "Example: Please revise the work according to user instructions and upload corrected file.",
                      hintStyle: TextStyle(
                ...
                SizedBox(
                  ...
                  child: ElevatedButton.icon(
                    onPressed: isLoading
                      ...
                      if (note.isEmpty) {
                        showSnack("Admin note is required", orange);
                        return;
                      }
                      ...
                      final success = await reviseDisputeApi(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: FutureBuilder<AdminDisputeOrderDetailsModel>(
            ...
            child: ListView(
              ...
              children: [
                ...
                section(
                  ...
                  children: [
                    infoTile("Dispute ID", "#DSP${data.disputeId}"),
                    infoTile("Order ID", "#ORD${data.orderId}"),
                    infoTile("Dispute Status", data.disputeStatus),
                    infoTile("Reason", data.disputeReason),
                    infoTile("User Message", data.disputeMessage),
                    infoTile(
                      "Order Status At Dispute",
                      data.orderStatusAtDispute,
                    ),
                    infoTile(
                    ...
                    infoTile(
                      "Dispute Created At",
                      formatDateTime(data.disputeCreatedAt),
                    ),
                  ],
                ),
                const SizedBox(height: 12),
                section(
                  ...
                  children: [
                    ...
                    infoTile("Status Label", data.statusLabel),
  ...
  Widget topDisputeCard(AdminDisputeOrderDetailsModel data) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              circleIcon(Icons.report_problem_outlined, red),
              const SizedBox(width: 10),
              Expanded(
                child: Text(
                  data.disputeReason.isEmpty
                      ? "Dispute Raised"
                      : data.disputeReason,
                  style: TextStyle(
                    color: textColor,
          ...
          Wrap(
            ...
            runSpacing: 8,
            children: [
              chip("#DSP${data.disputeId}", red),
              chip("#ORD${data.orderId}", primary),
              chip(
                data.disputeStatus.isEmpty ? "OPEN" : data.disputeStatus,
                orange,
              ),
            ],
          ),
  ...
  Widget quickFilesCard(AdminDisputeOrderDetailsModel data) {
  ...
  Widget submissionHistoryCard(AdminDisputeOrderDetailsModel data) {
    return Material(
      color: Colors.transparent,
      child: InkWell(
  ...
  Widget actionButtons(
      AdminDisputeOrderDetailsModel data,
      ) {
    return Container(
            ...
            children: [
              Expanded(
                child: SizedBox(
                  ...
                  child: OutlinedButton.icon(
                    onPressed: () {
                      showSnack("Next step: Reject dispute API", red);
                    },
              ...
              Expanded(
                child: SizedBox(
                  ...
                  child: ElevatedButton.icon(
                    onPressed: () {
                      showSnack("Next step: Resolve dispute API", green);
                    },
