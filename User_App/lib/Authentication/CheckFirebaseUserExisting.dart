...
class _CheckfirebaseuserexistingState extends State<Checkfirebaseuserexisting> {
  ...
  Future<void> _createOrLoginUser() async {
    ...
    try {
      ...
      try {
        ...
        if (e.code == "email-already-in-use") {
          debugPrint("Email already exists, trying login...");
      ...
      debugPrint("Server/Unexpected Error: $e");
  ...
  Future<bool> canCreateAccount() async {
    ...
    final uri = Uri.parse(
      "${apiConfig.baseUrl}/api/user_side/auth/public/canCreateAccount",
    ).replace(
      queryParameters: {
        "deviceId": deviceId,
    ...
    debugPrint("canCreateAccount URL: $uri");
    ...
    debugPrint("canCreateAccount Status: ${res.statusCode}");
    debugPrint("canCreateAccount Body: ${res.body}");
