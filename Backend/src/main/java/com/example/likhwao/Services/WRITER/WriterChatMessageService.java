...
public class WriterChatMessageService {
    ...
    private void validateOrderChatAccess(
        ...
        if (order.getWriterFirebaseUid() == null ||
                !order.getWriterFirebaseUid().equals(senderFirebaseUid)) {
            throw new RuntimeException("You are not allowed to chat for this order");
        }

        if (order.getUserFirebaseUid() == null ||
                !order.getUserFirebaseUid().equals(request.getReceiverFirebaseUid())) {
    ...
    private void saveMessageToFirebase(
        ...
        try {
            ...
            messageData.put("senderId", "WRITER_" + senderFirebaseUid);
            messageData.put("receiverId", "USER_" + request.getReceiverFirebaseUid());
            ...
            messageData.put("senderRole", "WRITER");
            messageData.put("receiverRole", "USER");
